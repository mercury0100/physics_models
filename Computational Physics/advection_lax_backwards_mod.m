...
% Additional elements for boundary conditions
D(1,1)=2;
D(1,2)=0;
D(L,L)=2;
D(L,L-1)=0;
...
for n=1:nsteps
    ...
    k=floor(time(n+1)/h); % The index the peak has reached
...
% Plot the amplitude versus position, the initial amplitude
    % profile, and the analytic values, with annotations
    figure(3);
    plot(x,amp_xt(:,floor(nsteps/2)));
    xlabel('Position (non-dim.)');
    ylabel('Amplitude (non-dim.)');
    title(['\tau/\tau_{\rm max}= ',num2str(tau/(h/c)),...
        '     Time: ',num2str(time(floor(nsteps/2)))]);
